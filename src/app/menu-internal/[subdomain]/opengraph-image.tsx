...
export default async function Image({
  ...
  try {
    ...
    if (!org?.banner) {
      const fallbackBackground = await loadPublicAssetDataUrl("og-image-bg.jpg")
      const orgLogoDataUrl = org?.logo
        ? await loadRemoteAssetDataUrl(org.logo).catch(() => null)
        : null

      return renderOgImage({
        backgroundSrc: fallbackBackground,
        brandLogoDataUrl: brandLogo,
        fontData,
        orgLogoSrc: orgLogoDataUrl,
        orgName
      })
    }

    return renderOgImage({
      backgroundSrc: org.banner,
      brandLogoDataUrl: brandLogo,
      fontData,
      orgLogoSrc: org.logo,
      orgName
